 ...
 * @version 0.9
 */
public class Yahtzee extends AbstractRule {
    /**
     * Creates a new rule object.
     *
     * @param rule type of this object.
     */
    public Yahtzee(Rule rule) {
        super(rule);
    }

    /**
     * Method calculates the value for this rule based on the fact wether dices build Yahtzee or not.
     * This Method checks, if the rule YAHTZEE has already been set once.
     ...
     * dices is stored. Then it is checked, if the number 3 or higher occurs in the result of
     * the method call testOfAKind. If so, one dice value occured at least 3 times in the
    ...
    public void calculateValue(List<Dice> cup) throws RuleNotAvailableException {
        ...
        if (getValue() != -1) {
            ...
            if (cup.get(0).equals(cup.get(AMOUNT_OF_FOUR_OF_A_KIND))) {
                sum = VALUE_OF_YAHTZEE;
            }
        }
        setValue(sum);
    }
